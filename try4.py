...
### for 10-K.csv
n=300#depends on how many 10-K report do we want
...
for i in L:
    ...
    try:
        dic=Rawtext().location(download)
    except AssertionError:
        continue
    
    x=['7A','7','8']#the order of items also matter!778
    #if change change assert also
    table=Rawtext.spcitems(dic,x)
    
    try:
        assert len(table.item)==3
    except:
        continue
    ...
    try:
        ...
        #write down
        Rawtext.writedown(item_7,main_dir+suffix2)
        Rawtext.writedown(item_7a,main_dir+suffix2)
...
for i in range(len(x)):
    ...
    #no stemming, no stopwords，no lemmatization!
    #but McDonald's list cover that parts.
